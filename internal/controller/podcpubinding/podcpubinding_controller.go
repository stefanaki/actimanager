...
func (r *PodCPUBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if cpuBinding.Status.ResourceStatus != v1alpha1.StatusValidated {
		...
		if !ok {
			...
			r.Recorder.Event(cpuBinding, corev1.EventTypeNormal, string(v1alpha1.StatusValidated), "CPU binding is validated")
		}
	}
	...
	err = r.applyCPUPinning(ctx, cpuSet, memSet, pod, ip)
	if err != nil {
